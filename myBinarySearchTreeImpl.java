...
public class myBinarySearchTreeImpl<T1 extends Comparable<? super T1>, T2> implements myBinarySearchTree<T1, T2> {
	...
	public myBinarySearchTree<T1, T2> my_left_tree() throws myException{
		if (this.root != null){
			myBinarySearchTree<T1, T2> ans = new myBinarySearchTreeImpl<T1, T2>(this.root.getLeft());
			return ans;
		}
		else
	...
	public myBinarySearchTree<T1, T2> my_right_tree() throws myException{
		if (this.root != null){
			myBinarySearchTree<T1, T2> ans = new myBinarySearchTreeImpl<T1, T2>(this.root.getRight());
			return ans;
		}
		else
			throw new myException("The tree is empty, so there is no left subtree");			
	...
	public myBinarySearchTree<T1, T2> my_insert(T1 key, T2 info){
		...
		//2 We make ans to point to the root of the tree and we return it
		ans = new myBinarySearchTreeImpl<T1, T2>(this.my_root()); 			
		return ans;				
	}
	...
	public int my_count_at_level(int level){
		int ans = 0;
			...
			if(!my_is_empty() && level > 0) {
				if(level == 1) {
					ans++;
				}
				
				ans += my_left_tree().my_count_at_level(level-1);
				ans	+= my_right_tree().my_count_at_level(level-1);
			}
	...
	public int my_count_smaller_nodes(T1 key){
		...
		if(!my_is_empty()) {
			...
			if(!my_left_tree().my_is_empty()) {
				...
				ans += my_left_tree().my_count_smaller_nodes(key);
			}
			...
			if(!my_right_tree().my_is_empty()) {
				...
				ans += my_right_tree().my_count_smaller_nodes(key);
			}
	...
	public int my_find_node_at_level(T1 key){
		...
		if(!my_is_empty()) {
			...
			case -1: 
				if(ans == 0) {
					if(!my_left_tree().my_is_empty()) {
						ans += my_left_tree().my_find_node_at_level(key);
						if(ans > 0) {
							ans++;	}
					}
					}
			...
			case 1:
				if(ans == 0) {
					if(!my_right_tree().my_is_empty()) {
						ans += my_right_tree().my_find_node_at_level(key);
						if(ans > 0) {
							ans++;
						}
						}					
